...
RUN apt-get update
RUN apt-get --fix-missing --no-install-recommends install procps openjdk-8-jdk git -y && \
    apt-get autoclean && apt-get --purge -y autoremove && \
    rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*

ADD http://www-us.apache.org/dist/maven/maven-3/3.6.3/binaries/apache-maven-3.6.3-bin.tar.gz /opt
WORKDIR /opt
RUN tar xvfz /opt/apache-maven-3.6.3-bin.tar.gz
...
ENTRYPOINT ["/opt/apache-maven-3.6.3/bin/mvn"]
